...
<p align="center">
  <strong>Open-source B2B ML-сервис для выбора лучшей модели на пользовательском time-to-event датасете с цензурированными наблюдениями.</strong>
</p>
...
<p align="center">
  ...
  <a href="#кому-нужен-проект">Для кого</a> ·
  <a href="#как-добавить-свой-датасет">Свой датасет</a> ·
...
SAWrap - open-source проект с B2B-сценарием применения. Он помогает выбрать лучшую
модель для пользовательского датасета, где нужно прогнозировать терминальное событие:
отказ оборудования, уход клиента, дефолт, смерть, рецидив или другое событие, после
которого дальнейшее наблюдение меняет смысл.

Проект решает проблему сравнения моделей, которые изначально отвечают на разные
вопросы. Классификация оценивает вероятность события, регрессия предсказывает время
до события, а модели анализа выживаемости строят функцию выживаемости. SAWrap
приводит эти ответы к единому survival-представлению и сравнивает модели в одной
системе метрик.
...
| Пользовательские данные | Загрузка CSV/CSV.GZ/ZIP/XLSX, стандартизация `time`/`cens`, расчет моделей и демо-прогноз |

---

## Почему это важно

На практике многие команды используют привычные модели классификации и регрессии
даже там, где задача является time-to-event задачей. В таких данных почти всегда
есть цензурированные наблюдения: объект наблюдался до некоторого момента, событие
не наступило, и точное будущее время события неизвестно.

Обычная классификация и регрессия работают с такими наблюдениями некорректно:

| Тип модели | Что возвращает | Проблема на цензурированных данных |
|---|---|---|
| Классификация | Вероятность события | Может принять цензурированный объект за отсутствие события и теряет информацию о времени |
| Регрессия | Числовое время события | Часто требует точного времени события и плохо использует неполные наблюдения |
| Анализ выживаемости | Функцию выживаемости | Учитывает цензурирование напрямую |

SAWrap предлагает практический pipeline: загрузить датасет, привести разные модели
к функции выживания, сравнить их по единым метрикам и выбрать подходящую модель для
...
SAWrap полезен исследователям, студентам и Data Scientists, которые хотят:

- улучшить свои модели на задачах с цензурированными наблюдениями;
- сравнить классификацию, регрессию и survival analysis в одном pipeline;
- использовать готовые обертки `ClassifWrapSA`, `RegrWrapSA`, `SAWrapSA` и Piecewise-обертки;
- воспроизвести экспериментальный стенд и расширить его своими моделями;
- быстро получить объяснение метрик через AI/RAG-компоненты.

### B2B

Проект ориентирован на компании, которым нужно выбрать лучшую модель для своего
датасета, где важно прогнозировать терминальные события:

- отказ оборудования или узла;
- отток клиента;
- дефолт или другое финансовое событие;
- медицинский или страховой риск;
- завершение жизненного цикла пользователя, устройства или договора.

Для такого сценария SAWrap работает как витрина и экспериментальный pipeline:
компания загружает свой датасет, выбирает метрики и модели, запускает расчет,
сравнивает результаты и получает демо-прогноз по вручную введенным признакам.

---

## Как добавить свой датасет

Загрузка пользовательского датасета доступна при локальном запуске с включенным
пересчетом:
...
На главной странице нужно нажать `Добавить датасет` и выбрать файл. Поддерживаются:
...
|---|---|
| Время наблюдения | Колонка времени: например `time`, `event_time`, `duration`, `tenure`, `tenure_months` |
| Индикатор события | Колонка события: например `cens`, `event`, `status`, `event_observed`, `churn`, `churn_value` |
| Кодировка события | `1`/`true`/`yes` - событие наступило, `0`/`false`/`no` - цензурированное наблюдение |
| Признаки | Хотя бы один полезный признак после удаления колонок времени и события |
| Качество строк | После очистки должно остаться минимум 5 строк |
| Вариативность | В событии должны быть оба класса, а время должно иметь хотя бы два разных значения |

Что делает загрузчик:

- приводит данные к стандартному виду `time`, `cens`, признаки;
- распознает числовые и категориальные признаки;
- заполняет простые пропуски в признаках;
- удаляет служебные, почти пустые и слишком высококардинальные текстовые поля;
- сохраняет стандартизированный датасет в `UI/user_datasets`;
- позволяет запустить расчет моделей и построить демо-прогноз на сайте.

Важно: `cens` в проекте означает индикатор наблюдаемого события в survival-массиве:
`True`/`1` - событие произошло, `False`/`0` - наблюдение цензурировано.
